...
@Serializable
data class UpgradesDto(val upgrades: List<UpgradeDto>)
...
fun toGameConfig(
    ...
    return GameConfig(
        ...
        unitDefs = unitDefs.units.map { unit ->
            UnitDef(
                id = unit.id,
                role = unit.role,
        ...
        enemyDefs = enemyDefs.enemies.map { enemy ->
            EnemyDef(
                id = enemy.id,
                hp = enemy.hp,
                speed = enemy.speed,
        ...
        upgrades = upgrades.upgrades.map { upgrade ->
            UpgradeDef(id = upgrade.id, type = upgrade.type, name = upgrade.name)
        },
